...
const PersonDetailsPage = () => {
  ...
  const tvCredits = {
    data: {
      results: useGetData("tvCredits", `person/${person_id}/tv_credits`, {}, 0)
        ?.data?.cast,
    },
    isLoading: useGetData("tvCredits", `person/${person_id}/tv_credits`, {}, 0)
      .isLoading,
  };
  useEffect(() => {
    const fetchHandler = async () => {
      const result = await fetchData(`person/${person_id}`);
      setData(result);
    };
    fetchHandler();
  }, []);
    ...
    <>
      ...
      <div className="w-[100%] lg:flex flex-col items-center">
        ...
        <div className="w-screen flex flex-col mt-10">
          <CardSlider
            name={`${data.name}'s Shows`}
            data={tvCredits}
            dataType={"tv"}
          />
          <CardSlider
            name={`${data.name}'s Movies`}
            data={movieCredits}
            dataType={"movie"}
          />
        </div>
